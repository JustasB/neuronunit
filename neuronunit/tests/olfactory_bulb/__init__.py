...
class OlfactoryBulbCellTest(VmTest):
    ...
    def get_dependent_prediction(self, dependent_test_class_generic, model):
        ...
        if len(mro) < 4:
            raise Exception("The test should be a class that inherits from an olfactory_bulb.publications class"
                            "AND from a generic olfactory_bulb.tests class, in that order. E.g. "
